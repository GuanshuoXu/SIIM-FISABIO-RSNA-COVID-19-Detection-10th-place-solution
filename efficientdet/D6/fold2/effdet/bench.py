...
def _post_process(
    ...
    #print(cls_outputs[0].permute(0, 2, 3, 1).shape, "aaa")
    #print(cls_outputs[0].permute(0, 2, 3, 1).reshape([batch_size, -1, num_classes]).shape, "bbb")
    cls_outputs_all = torch.cat([
        cls_outputs[level].permute(0, 2, 3, 1).reshape([batch_size, -1, num_classes])
        for level in range(num_levels)], 1)
    #print(cls_outputs_all.shape, "AAA")
    box_outputs_all = torch.cat([
        box_outputs[level].permute(0, 2, 3, 1).reshape([batch_size, -1, 4])
        for level in range(num_levels)], 1)

    #print(cls_outputs_all.reshape(batch_size, -1).shape, "BBB")
    _, cls_topk_indices_all = torch.topk(cls_outputs_all.reshape(batch_size, -1), dim=1, k=max_detection_points)
    #print(cls_topk_indices_all.shape, cls_topk_indices_all.dtype, cls_topk_indices_all.min(), cls_topk_indices_all.max(), "CCC")
    ...
    #print(cls_outputs_all.shape, "FFF")
    cls_outputs_all_after_topk = torch.gather(
        cls_outputs_all, 1, indices_all.unsqueeze(2).expand(-1, -1, num_classes))
    #print(indices_all.unsqueeze(2).shape, indices_all.unsqueeze(2).expand(-1, -1, num_classes).shape, "GGG")
    #print(cls_outputs_all_after_topk.shape, "HHH")
    cls_outputs_all_after_topk = torch.gather(
        cls_outputs_all_after_topk, 2, classes_all.unsqueeze(2))
    #print(classes_all.unsqueeze(2).shape, "III")
    #print(cls_outputs_all_after_topk.shape, "AAA")
    #print(box_outputs_all_after_topk.shape, "BBB")
    #print(indices_all.shape, indices_all.dtype, indices_all.min(), indices_all.max(), "DDD")
    #print(classes_all.shape, classes_all.dtype, classes_all.min(), classes_all.max(), "EEE")
...
class DetBenchTrain(nn.Module):
    ...
    def forward(self, x, target: Dict[str, torch.Tensor]):
        ...
        #print(class_out[0].shape, box_out[0].shape)
        #print(class_out[1].shape, box_out[1].shape)
        #print(class_out[2].shape, box_out[2].shape)
        #print(class_out[3].shape, box_out[3].shape)
        #print(class_out[4].shape, box_out[4].shape)
        #print(class_out[0][0,:,2,2])
        #print(gg)
