...
class VinDataset(Dataset):
    def __init__(self, fold, img_ids, label_dict, image_list, target_size):
        parser_config = CocoParserCfg(ann_filename='../../splits/split4/valid_original_fold{}.json'.format(fold))
...
def main():
    ...
    for fold in range(5):
        ...
        import pickle
        with open('../../splits/split4/valid_image_id_cv_list.pickle', 'rb') as f:
            image_list_valid = pickle.load(f)[fold]
        with open('../../splits/split4/label_dict.pickle', 'rb') as f:
            label_dict = pickle.load(f)
        with open('../../splits/split4/img_ids_fold{}.pickle'.format(fold), 'rb') as f:
