...
class NoneDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'bbox_list' in self.label_dict[self.id_list[index]]:
            ...
            y_mask = cv2.resize(self.label_dict[self.id_list[index]]['mask'], (x.shape[0], x.shape[1])) 
        ...
        if 'is_none' in self.label_dict[self.id_list[index]]:
            if self.label_dict[self.id_list[index]]['is_none']:
                y = 1.
            else:
                y = 0.
        else:
            y = self.label_dict[self.id_list[index]]['none']
        return x, y, y_mask
...
def main():
    ...
    with open('pseudo_labels/id_list_midrc.pickle', 'rb') as f:
        id_list += pickle.load(f)
    with open('pseudo_labels/id_list_bimcv.pickle', 'rb') as f:
        id_list += pickle.load(f)
    with open('pseudo_labels/id_list_bimcv_negative_old.pickle', 'rb') as f:
        id_list += pickle.load(f)
    with open('../../splits/split1/label_dict.pickle', 'rb') as f:
        label_dict = pickle.load(f)
    with open('pseudo_labels/label_dict_midrc.pickle', 'rb') as f:
        label_dict.update(pickle.load(f))
    with open('pseudo_labels/label_dict_bimcv.pickle', 'rb') as f:
        label_dict.update(pickle.load(f))
    with open('pseudo_labels/label_dict_bimcv_negative_old.pickle', 'rb') as f:
        label_dict.update(pickle.load(f))
    ...
    if args.local_rank == 0:
        torch.distributed.barrier()
    if args.local_rank == 0:
        model.load_state_dict(torch.load('weights/epoch5'))
    ...
    train_datagen = NoneDataset(id_list, label_dict, train_transform)
    ...
    for ep in range(6,num_epoch):
        ...
        for j, (images, labels, labels_mask) in enumerate(train_generator):
            ...
            with autocast():
                ...
                loss = criterion(logits.squeeze(), labels.squeeze())+32*criterion(logits_mask.squeeze(), labels_mask.squeeze())
