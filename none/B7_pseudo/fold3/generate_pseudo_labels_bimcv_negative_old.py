...
def main():
    ...
    for fold in range(3,4):
        # prepare input
        import glob
        files = sorted(glob.glob('../../../processed_input/bimcv_negative_old/train/*.png'))
        label_dict = {}
        valid_id_list = []
        ...
        for ckp in checkpoint_list:
            ...
            # build model
            model = NoneEffb7()
            model.load_state_dict(torch.load('../../B7/fold{}/weights/'.format(fold)+ckp))
            model.cuda()
            ...
            pred_prob_mask_valid = np.zeros((len(valid_id_list), 42, 42), dtype=np.float32)
        ...
        for i in range(len(valid_id_list)):
            label_dict[valid_id_list[i]]['none'] = pred_prob_valid[i]
            label_dict[valid_id_list[i]]['mask'] = pred_prob_mask_valid[i]

        out_dir = 'pseudo_labels/'
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        with open(out_dir+'id_list_bimcv_negative_old.pickle', 'wb') as f:
            pickle.dump(valid_id_list, f, protocol=pickle.HIGHEST_PROTOCOL)
        with open(out_dir+'label_dict_bimcv_negative_old.pickle', 'wb') as f:
