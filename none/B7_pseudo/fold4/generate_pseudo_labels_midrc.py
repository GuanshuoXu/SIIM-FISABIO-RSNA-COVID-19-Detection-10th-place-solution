...
def main():
    ...
    for fold in range(4,5):
        # prepare input
        import glob
        files = sorted(glob.glob('../../../external_data/MIDRC/MIDRC-RICORD/MIDRC-RICORD/*.jpg'))
        label_dict = {}
        valid_id_list = []
        ...
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        with open(out_dir+'id_list_midrc.pickle', 'wb') as f:
            pickle.dump(valid_id_list, f, protocol=pickle.HIGHEST_PROTOCOL)
        with open(out_dir+'label_dict_midrc.pickle', 'wb') as f:
